<template>
    <div class="login">
        ...
        <div class="login-content">
            <div>
                ...
                <div class="login-input">
                    <div>
                        <input name="user_name" placeholder="请输入手机号" v-model="phone"/><span id="code"  :class="{active:/^(13[0-9]|14[5|7]|15[0|1|2|3|5|6|7|8|9]|18[0|1|2|3|5|6|7|8|9])\d{8}$/.test(phone),redActive:countDown!='获取验证码'}">{{countDown}}</span>
                    </div>
                    <div>
        ...
        <div class="codeError" v-if="errorShow">
            <img src="../assets/images/errorCode.svg" />
            <p>验证码错误</p>
        </div>
    </div>
</template>
...
export default {
    ...
    methods:{
        ...
        login(){
            if(/^(13[0-9]|14[5|7]|15[0|1|2|3|5|6|7|8|9]|18[0|1|2|3|5|6|7|8|9])\d{8}$/.test(this.phone)){
                if(this.code){
                    ...
                    let code=this.code;              
                    axios.post('http://101.37.76.177:8000/login',{phone:this.phone,user_nickname:nickname,code:code})
                    .then(res=>{
        ...
        getCode(){
            if(/^(13[0-9]|14[5|7]|15[0|1|2|3|5|6|7|8|9]|18[0|1|2|3|5|6|7|8|9])\d{8}$/.test(this.phone)){
                    ...
                    if(this.count>0){
                        //移除获取验证码事件
                        document.getElementById("code").removeEventListener("click",this.getCode);
                        this.countDown=this.count+"秒后再次获取";
                        this.count--;
                    }else if(this.count==0){
                        //一分钟后重新绑定获取验证码事件
                        document.getElementById("code").addEventListener("click",this.getCode,true)
                        this.countDown="获取验证码";
                        clearInterval(timer)
                    }
                },1000)
                axios.post('http://101.37.76.177:8000/send',{phone:this.phone})
                .then(res=>{
                    console.log(res)
                })
            }
    ...
    mounted(){
        ...
        //给获取验证码绑定点击事件
        document.getElementById("code").addEventListener("click",this.getCode)
    }
}
</script>
<style scoped>
    ...
    .codeError{width:3rem;height:3rem;border-radius: .5rem;background: rgba(0,0,0,.7);text-align: center;color:#fff;font-size: 14px;position: fixed;top:50%;left:50%;margin-top:-1.5rem;margin-left:-1.5rem;}
    .codeError>img{width:2rem;height:2rem;margin:.1rem 0}
    .codeError>p{color:#fff;}
</style>
